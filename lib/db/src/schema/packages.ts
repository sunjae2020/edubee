...
export const packageGroups = pgTable("package_groups", {
  ...
  instituteId:        uuid("institute_id").references(() => accounts.id, { onDelete: "set null" }),
  accommodation:      varchar("accommodation", { length: 255 }),
  accommodationId:    uuid("accommodation_id").references(() => accounts.id, { onDelete: "set null" }),
  tourCompany:        varchar("tour_company", { length: 255 }),
  tourCompanyId:      uuid("tour_company_id").references(() => accounts.id, { onDelete: "set null" }),
  pickupDriver:       varchar("pickup_driver", { length: 255 }),
  pickupDriverId:     uuid("pickup_driver_id").references(() => accounts.id, { onDelete: "set null" }),
  requiredDocuments:  text("required_documents"),
  packagePptUrl:      text("package_ppt_url"),
