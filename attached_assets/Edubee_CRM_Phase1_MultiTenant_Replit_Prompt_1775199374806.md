...
# Replit AI Agent 전용
# 작성일: 2026-04-03 | 버전: v1.1 (온보딩 시드 로직 추가)
...
Task 5 → 온보딩 시드 로직 (신규 테넌트 자동 초기 데이터 생성)
```
...
> 🎯 목적: 신규 테넌트 생성 시 CRM이 즉시 사용 가능하도록
> 필수 마스터 데이터를 자동으로 생성한다.
> 기존 테넌트(Time Study) 데이터는 절대 건드리지 않는다.

### 5-1. 온보딩 시드 데이터 전체 구조

```
신규 테넌트 생성 (organisations INSERT)
        │
        ▼
onboardTenant(organisationId) 함수 자동 호출
        │
        ├─── ① Tax Rates          (세금 설정 3종)
        ├─── ② Product Groups     (상품 그룹 7종)
        ├─── ③ Product Types      (상품 유형 14종 — 서비스 모듈 연결)
        ├─── ④ Cost Centers       (레거시 계정과목 5종)
        ├─── ⑤ Chart of Accounts  (회계 계정과목표 18종)
        ├─── ⑥ Payment Info       (기본 수금 계좌 1종)
        └─── ⑦ Default Team       (기본 Admin 팀 1개)
```

### 5-2. 온보딩 서비스 파일 생성

`/server/src/services/onboardingService.ts` 파일을 **신규 생성**한다.

#### 핵심 원칙
- 모든 INSERT에 `organisation_id` 컬럼을 반드시 포함한다
- `ON CONFLICT DO NOTHING`으로 중복 실행 방지
- 트랜잭션(transaction) 1개로 전체를 묶어 원자성 보장
  → 중간 실패 시 전체 롤백, 부분 생성 상태 없음
- 시드 완료 후 `organisations.onboarded_at = NOW()` 업데이트
...
export async function onboardTenant(organisationId: string): Promise<void> {
  await db.transaction(async (tx) => {
    ...
    // ① Tax Rates 생성
    await seedTaxRates(tx, organisationId);

    // ② Product Groups 생성
    const pgIds = await seedProductGroups(tx, organisationId);

    // ③ Product Types 생성 (Product Groups ID 참조)
    await seedProductTypes(tx, organisationId, pgIds);

    // ④ Cost Centers 생성 (레거시)
    await seedCostCenters(tx, organisationId);

    // ⑤ Chart of Accounts 생성
    await seedChartOfAccounts(tx, organisationId);

    // ⑥ Payment Info 생성
    await seedPaymentInfo(tx, organisationId);
...
```typescript
async function seedTaxRates(tx, organisationId: string) {
...
#### ② seedProductGroups — 상품 그룹 (7종)

```typescript
async function seedProductGroups(tx, organisationId: string) {
  // INSERT 후 생성된 ID를 다음 단계(Product Types)에서 참조
  const groups = await tx.insert(productGroups).values([
    { organisationId, name: 'Language Course',    description: '어학 과정',          status: 'Active' },
    { organisationId, name: 'Academic Program',   description: '학위 및 학업 과정',   status: 'Active' },
    { organisationId, name: 'Vocational (VET)',   description: '직업 교육 훈련',      status: 'Active' },
    { organisationId, name: 'Short Course',       description: '단기 연수 과정',      status: 'Active' },
    { organisationId, name: 'Camp Program',       description: '캠프 프로그램',       status: 'Active' },
    { organisationId, name: 'Placement Service',  description: '배치 및 지원 서비스', status: 'Active' },
    { organisationId, name: 'Support Service',    description: '생활 지원 서비스',    status: 'Active' },
  ]).returning({ id: productGroups.id, name: productGroups.name });

  // name → id 매핑 객체로 반환 (Product Types 시드에서 사용)
  return Object.fromEntries(groups.map(g => [g.name, g.id]));
}
```

#### ③ seedProductTypes — 상품 유형 14종 (서비스 모듈 자동 활성화 트리거 포함)

```typescript
async function seedProductTypes(tx, organisationId: string, pgIds: Record<string, string>) {
  await tx.insert(productTypes).values([
    ...
    // Language Course 하위
    {
      ...
      productGroupId: pgIds['Language Course'],
      serviceModuleType: 'study_abroad',   // → study_abroad_mgt 자동 생성
      status: 'Active',
    },
    {
      organisationId,
      name: 'IELTS / Cambridge Prep',
      productGroupId: pgIds['Language Course'],
      serviceModuleType: 'study_abroad',
      status: 'Active',
    },
    ...
    // Academic Program 하위
    {
      ...
      productGroupId: pgIds['Academic Program'],
      serviceModuleType: 'study_abroad',
      status: 'Active',
    },
    {
    ...
    {
      organisationId,
      name: 'Certificate / Diploma (VET)',
      productGroupId: pgIds['Vocational (VET)'],
      serviceModuleType: 'study_abroad',
      status: 'Active',
    },
    {
    ...
    // Short Course 하위
    {
      ...
      productGroupId: pgIds['Short Course'],
      serviceModuleType: 'study_abroad',
      status: 'Active',
    },
    ...
    // Camp Program 하위
    {
      organisationId,
      name: 'Camp Program',
      productGroupId: pgIds['Camp Program'],
      serviceModuleType: 'camp',           // → camp 모듈 자동 연결
      status: 'Active',
    },

    // Placement Service 하위
    {
      organisationId,
      name: 'Airport Pickup',
      productGroupId: pgIds['Placement Service'],
      serviceModuleType: 'pickup',         // → pickup_mgt 자동 생성
      status: 'Active',
    },
    {
      organisationId,
      name: 'Accommodation (Homestay)',
      productGroupId: pgIds['Placement Service'],
      serviceModuleType: 'accommodation',  // → accommodation_mgt 자동 생성
      status: 'Active',
    },
    {
      organisationId,
      ...
      productGroupId: pgIds['Placement Service'],
      serviceModuleType: 'internship',     // → internship_mgt 자동 생성
      status: 'Active',
    },
    ...
    // Support Service 하위
    {
      ...
      productGroupId: pgIds['Support Service'],
      serviceModuleType: 'settlement',     // → settlement_mgt 자동 생성
      status: 'Active',
    },
    {
      organisationId,
      ...
      productGroupId: pgIds['Support Service'],
      serviceModuleType: 'guardian',       // → guardian_mgt 자동 생성
      status: 'Active',
    },
...
#### ④ seedCostCenters — 레거시 계정과목 (5종)

```typescript
async function seedCostCenters(tx, organisationId: string) {
  await tx.insert(costCenters).values([
    { organisationId, name: 'Tuition Fee',      code: 'TF',  category: 'Revenue',  status: 'Active' },
    { organisationId, name: 'Service Fee',       code: 'SF',  category: 'Revenue',  status: 'Active' },
    { organisationId, name: 'Commission',        code: 'COM', category: 'Expense',  status: 'Active' },
    { organisationId, name: 'Agent Commission',  code: 'AC',  category: 'Expense',  status: 'Active' },
    { organisationId, name: 'Operating Expense', code: 'OE',  category: 'Expense',  status: 'Active' },
  ]).onConflictDoNothing();
}
```

#### ⑤ seedChartOfAccounts — 회계 계정과목표 18종

```typescript
// ⚠️ chart_of_accounts는 organisation_id 없이 공유 사용하는 경우 주의
// organisation_id 컬럼이 있으면 테넌트별 생성, 없으면 글로벌 공유 사용
// 스키마 확인 후 결정한다

async function seedChartOfAccounts(tx, organisationId: string) {
  await tx.insert(chartOfAccounts).values([

    // ASSETS (자산) — 1xxx
    { code: `${organisationId.slice(0,4)}-1100`, name: 'Trust Account — AUD',         accountType: 'asset',     description: '수탁 은행 계좌' },
    { code: `${organisationId.slice(0,4)}-1200`, name: 'Operating Account — AUD',     accountType: 'asset',     description: '운영 은행 계좌' },
    { code: `${organisationId.slice(0,4)}-1300`, name: 'Accounts Receivable',         accountType: 'asset',     description: '미수금' },

    // LIABILITIES (부채) — 2xxx
    { code: `${organisationId.slice(0,4)}-2100`, name: 'Trust Liability — Students',  accountType: 'liability', description: '학생 수탁금 부채' },
    { code: `${organisationId.slice(0,4)}-2200`, name: 'GST Collected',               accountType: 'liability', description: 'GST 수집액' },
    { code: `${organisationId.slice(0,4)}-2300`, name: 'Commission Payable',          accountType: 'liability', description: '미지급 커미션' },

    // REVENUE (수익) — 3xxx
    { code: `${organisationId.slice(0,4)}-3100`, name: 'Service Fee Revenue',         accountType: 'revenue',   description: '서비스 수수료 수익' },
    { code: `${organisationId.slice(0,4)}-3200`, name: 'Commission Revenue',          accountType: 'revenue',   description: '커미션 수익' },
    { code: `${organisationId.slice(0,4)}-3300`, name: 'Accommodation Revenue',       accountType: 'revenue',   description: '홈스테이 수익' },
    { code: `${organisationId.slice(0,4)}-3400`, name: 'Pickup Revenue',              accountType: 'revenue',   description: '픽업 서비스 수익' },
    { code: `${organisationId.slice(0,4)}-3500`, name: 'Camp Revenue',                accountType: 'revenue',   description: '캠프 수익' },

    // COGS (매출원가) — 4xxx
    { code: `${organisationId.slice(0,4)}-4100`, name: 'Sub-Agent Commission',        accountType: 'cogs',      description: '하위 에이전트 커미션' },
    { code: `${organisationId.slice(0,4)}-4200`, name: 'Super-Agent Commission',      accountType: 'cogs',      description: '상위 에이전트 커미션' },
    { code: `${organisationId.slice(0,4)}-4300`, name: 'Referral Fee',                accountType: 'cogs',      description: '소개 수수료' },
    { code: `${organisationId.slice(0,4)}-4400`, name: 'School Transfer',             accountType: 'cogs',      description: '학교 납부금' },

    // EXPENSES (비용) — 5xxx
    { code: `${organisationId.slice(0,4)}-5100`, name: 'Staff Incentive',             accountType: 'expense',   description: '직원 인센티브' },
    { code: `${organisationId.slice(0,4)}-5200`, name: 'Accommodation Cost',          accountType: 'expense',   description: '숙박 비용' },
    { code: `${organisationId.slice(0,4)}-5300`, name: 'Operating Expense',           accountType: 'expense',   description: '운영비' },

  ]).onConflictDoNothing();
}
```

> ⚠️ **chart_of_accounts 스키마 확인 필수**
> 현재 스키마에 `organisation_id` 컬럼이 없다면, 코드값에 접두사를 붙이는 대신
> 글로벌 공유 테이블로 사용하고 시드를 1회만 실행한다.
> 스키마를 먼저 읽고 방식을 결정한다.

#### ⑥ seedPaymentInfo — 기본 수금 계좌 (1종)

```typescript
async function seedPaymentInfo(tx, organisationId: string) {
  await tx.insert(paymentInfos).values([
    {
      organisationId,
      name: 'Default Trust Account',
      accountType: 'Trust',
      currency: 'AUD',
      isDefault: true,
      status: 'Active',
      // 실제 계좌 정보는 Settings → Company Profile에서 관리자가 직접 입력
    },
  ]).onConflictDoNothing();
}
```

#### ⑦ seedDefaultTeam — 기본 팀 생성 (1개)

```typescript
async function seedDefaultTeam(tx, organisationId: string) {
  await tx.insert(teams).values([
    {
      organisationId,
      name: 'Admin Team',
      description: '기본 관리자 팀 — 온보딩 시 자동 생성',
      status: 'Active',
    },
  ]).onConflictDoNothing();
}
...
`/server/src/routes/superadmin.ts`의 테넌트 생성 API에 연결한다:

```typescript
// POST /api/superadmin/tenants — 신규 테넌트 생성
router.post('/tenants', superAdminOnly, async (req, res) => {
  const { name, subdomain, ownerEmail, planType } = req.body;

  // 1. organisations 테이블에 INSERT
  const [newOrg] = await db.insert(organisations).values({
    ...
    planStatus: 'trial',
    trialEndsAt: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000), // 14일 트라이얼
    status: 'Active',
  }).returning();

  // 2. 온보딩 시드 실행 (트랜잭션으로 원자성 보장)
  await onboardTenant(newOrg.id);

  // 3. 관리자 초대 이메일 발송 (MVP: 콘솔 로그로 대체)
  console.log(`[INVITE] ${ownerEmail} → ${subdomain}.edubee.com`);

  res.status(201).json({
    success: true,
    tenant: newOrg,
    message: `테넌트 생성 완료. ${ownerEmail}로 초대 이메일을 발송했습니다.`,
  });
});
```
...
테넌트의 시드 완료 여부를 확인하는 엔드포인트:

```typescript
// 응답 예시
{
  "organisationId": "uid-abc",
  "onboardedAt": "2026-04-03T10:00:00Z",
  "seedStatus": {
    "taxRates":         { "count": 3,  "ok": true },
    "productGroups":    { "count": 7,  "ok": true },
    "productTypes":     { "count": 14, "ok": true },
    "costCenters":      { "count": 5,  "ok": true },
    "chartOfAccounts":  { "count": 18, "ok": true },
    "paymentInfo":      { "count": 1,  "ok": true },
    "defaultTeam":      { "count": 1,  "ok": true }
  },
  "allComplete": true
}
```

### 5-6. Super Admin UI — 테넌트 생성 모달

`TenantList.tsx`에 **"+ New Tenant"** 버튼과 생성 모달을 추가한다.

모달 입력 필드:
```
[모달] Create New Tenant
  ─────────────────────────────────────
  Company Name *        (text input)
  Subdomain *           (text input) + ".edubee.com" suffix 표시
                        → 입력 시 실시간 중복 확인 (Check Availability API)
  Owner Email *         (text input, type="email")
                        → 이 이메일로 초대 링크 발송
  Plan                  (select: Starter | Professional | Enterprise)
  Trial Period          (select: 14일 | 30일 | 60일)
  ─────────────────────────────────────
  [Cancel]              [Create Tenant →]
  
  생성 중 로딩 상태:
    ① organisations 생성 중...
    ② 기본 데이터 설정 중... (Tax Rates, Products...)
    ③ 초대 이메일 발송 중...
    ✅ 완료! abc.edubee.com 가 생성되었습니다.
```
...
8. **온보딩 확인** → `POST /api/superadmin/tenants` 로 테스트 테넌트 생성
9. **온보딩 확인** → `GET /api/superadmin/tenants/:id/seed-status` 로 전체 시드 완료 확인
10. **DB 확인** → 생성된 테넌트의 `tax_rates`, `product_groups`, `product_types` 데이터 SELECT 확인
...
Task 4 — Super Admin
  ...
  □ TenantList.tsx (+ New Tenant 버튼 및 생성 모달 포함)
  □ SuperAdminGuard 컴포넌트
  □ App.tsx 라우트 등록

Task 5 — 온보딩 시드 로직
  □ /server/src/services/onboardingService.ts 생성
  □ seedTaxRates       (3종: GST 10%, Exempt, Zero-rated)
  □ seedProductGroups  (7종: Language, Academic, VET, Short, Camp, Placement, Support)
  □ seedProductTypes   (14종: service_module_type 매핑 포함)
  □ seedCostCenters    (5종: 레거시 계정과목)
  □ seedChartOfAccounts(18종: 자산/부채/수익/원가/비용)
  □ seedPaymentInfo    (1종: Default Trust Account)
  □ seedDefaultTeam    (1개: Admin Team)
  □ POST /api/superadmin/tenants 에 onboardTenant() 연결
  □ GET  /api/superadmin/tenants/:id/seed-status API 구현
  □ 테스트 테넌트 생성 후 seed-status 전체 ok 확인
```
...
*Edubee CRM — Phase 1 Multi-Tenant Prompt v1.1 (온보딩 시드 추가) | 2026-04-03*
