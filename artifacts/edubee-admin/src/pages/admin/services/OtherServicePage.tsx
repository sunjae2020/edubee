...
const STATUS_STYLE: Record<string, { bg: string; text: string }> = {
  ...
  confirmed:   { bg: "var(--e-orange-lt)", text: "var(--e-orange)" },
...
export default function OtherServicePage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["other-services", search, status, page, pageSize, sortBy, sortDir],
    queryFn: () => {
  ...
  const hardDelMutation = useMutation({
    ...
    onSuccess: (_d: any, ids: string[]) => { qc.invalidateQueries({ queryKey: ["other-services"] }); clearSelection(); toast({ title: `${ids.length} permanently deleted` }); },
