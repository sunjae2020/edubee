...
export default function ProductTypes() {
  ...
  const softDelMutation = useMutation({
    mutationFn: (ids: string[]) => axios.delete(`${BASE}/api/product-types/bulk`, { data: { ids, soft: true } }).then(r => r.data),
    onSuccess: (_d, ids) => { qc.invalidateQueries({ queryKey: ["product-types"] }); clearSelection(); toast({ title: `${ids.length} moved to trash` }); },
    onError: () => toast({ title: "삭제 실패", variant: "destructive" }),
  });
  const hardDelMutation = useMutation({
    mutationFn: (ids: string[]) => axios.delete(`${BASE}/api/product-types/bulk`, { data: { ids } }).then(r => r.data),
    onSuccess: (_d, ids) => { qc.invalidateQueries({ queryKey: ["product-types"] }); clearSelection(); toast({ title: `${ids.length} permanently deleted` }); },
    onError: () => toast({ title: "삭제 실패", variant: "destructive" }),
  });
  ...
  return (
    <div className="space-y-5">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#F4F3F1] bg-[#FAFAF9]">
              ...
              <SortableTh key="Product Group" col="groupName" sortBy={sortBy} sortDir={sortDir} onSort={onSort} className="text-left px-4 py-3 text-xs font-medium text-[#57534E] uppercase tracking-wide">Product Group</SortableTh>
              <SortableTh key="Service Module" col="serviceModule" sortBy={sortBy} sortDir={sortDir} onSort={onSort} className="text-left px-4 py-3 text-xs font-medium text-[#57534E] uppercase tracking-wide">Service Module</SortableTh>
          ...
          <tbody className="divide-y divide-[#F4F3F1]">
            {isLoading ? (
              ...
              <tr><td colSpan={isSA ? 7 : 6} className="px-4 py-16 text-center text-[#A8A29E] text-sm">No product types found</td></tr>
            ) : paged.map(t => (
              <tr
                key={t.id}
              ...
              >
                {isSA && <td className="px-3 py-3 w-10" onClick={e => e.stopPropagation()}><input type="checkbox" checked={selectedIds.has(t.id)} onChange={() => toggleSelect(t.id)} className="rounded border-stone-300" /></td>}
                ...
                <td className="px-4 py-3 text-[#A8A29E] text-xs">{t.createdOn ? formatDate(t.createdOn) : "—"}</td>
      ...
      <ListPagination page={page} pageSize={pageSize} total={sorted.length} onChange={setPage} onPageSizeChange={v => { setPageSize(v); setPage(1); }} label="types" />
