...
export default function ProductTypes() {
  ...
  const softDelMutation = useMutation({
    ...
    onSuccess: (_d, ids) => { qc.invalidateQueries({ queryKey: ["product-types"] }); clearSelection(); toast({ title: `${ids.length}개 임시 삭제됨` }); },
  ...
  const hardDelMutation = useMutation({
    ...
    onSuccess: (_d, ids) => { qc.invalidateQueries({ queryKey: ["product-types"] }); clearSelection(); toast({ title: `${ids.length}개 영구 삭제됨` }); },
    onError: () => toast({ title: "삭제 실패", variant: "destructive" }),
  });
