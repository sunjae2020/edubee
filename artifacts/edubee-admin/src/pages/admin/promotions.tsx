...
export default function Promotions() {
  ...
  const softDelMutation = useMutation({
    ...
    onSuccess: (_d, ids) => { qc.invalidateQueries({ queryKey: ["promotions"] }); clearSelection(); toast({ title: `${ids.length} moved to trash` }); },
  ...
  const hardDelMutation = useMutation({
    ...
    onSuccess: (_d, ids) => { qc.invalidateQueries({ queryKey: ["promotions"] }); clearSelection(); toast({ title: `${ids.length} permanently deleted` }); },
    onError: () => toast({ title: "삭제 실패", variant: "destructive" }),
  });
