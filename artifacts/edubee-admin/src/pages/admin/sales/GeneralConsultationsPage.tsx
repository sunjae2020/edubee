...
export default function GeneralConsultationsPage() {
  ...
  const queryKey = ["general-consultations", page, search, statusFilter];
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await axios.get(`${BASE}/api/general-consultations`, { params });
      return res.data as { data: GCRow[]; total: number; page: number; totalPages: number };
    },
  });
  ...
  const softDelMutation = useMutation({
    mutationFn: (ids: string[]) => axios.patch(`${BASE}/api/general-consultations/bulk/soft-delete`, { ids }),
    onSuccess: (_d, ids) => { qc.invalidateQueries({ queryKey: ["general-consultations"] }); clearSelection(); toast({ title: `${ids.length} moved to trash` }); },
  });

  const hardDelMutation = useMutation({
    mutationFn: (ids: string[]) => axios.delete(`${BASE}/api/general-consultations/bulk`, { data: { ids } }),
    onSuccess: (_d, ids) => { qc.invalidateQueries({ queryKey: ["general-consultations"] }); clearSelection(); toast({ title: `${ids.length} permanently deleted` }); },
  });
    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button onClick={() => navigate("/admin/sales/general-consultations/new")} className="flex items-center gap-1.5">
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b">
            <tr>
              ...
              <SortableTh sortKey="preferredDestination" state={sortState} className="px-4 py-3 text-left font-medium text-gray-600">Destination</SortableTh>
              <SortableTh sortKey="targetTimeline"      state={sortState} className="px-4 py-3 text-left font-medium text-gray-600">Timeline</SortableTh>
          ...
          <tbody>
            {isLoading ? (
              ...
              return (
                <tr key={c.id} className="border-t hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => navigate(`/admin/sales/general-consultations/${c.id}`)}>
                  {isSA && (
                    ...
                    {topics.length > 0 ? (
                      <div className="flex flex-wrap gap-1">
                        {topics.slice(0, 2).map((t, i) => (
                          <Badge key={i} className="text-xs bg-orange-50 text-orange-700 border-0">{t}</Badge>
                        ))}
                        {topics.length > 2 && <Badge className="text-xs bg-gray-100 text-gray-500 border-0">+{topics.length - 2}</Badge>}
                      </div>
                    ) : <span className="text-gray-400">—</span>}
                  </td>
                  <td className="px-4 py-3 text-gray-700">{c.preferredDestination ?? "—"}</td>
                  <td className="px-4 py-3 text-gray-700">{c.targetTimeline ?? "—"}</td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                    <Button variant="ghost" size="icon" className="w-8 h-8" onClick={() => navigate(`/admin/sales/general-consultations/${c.id}`)}>
      ...
      {showShareModal && (
        <ShareLinkModal
          ...
          publicUrl={`${window.location.origin}/website/forms/general-consultation`}
