    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {!isLoading && Object.keys(byType).length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {Object.entries(byType).map(([type, items]) => {
            ...
            return (
              <div key={type} className="rounded-xl p-4 border" style={{ background: "var(--e-bg-surface)", borderColor: "var(--e-border)", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                <div className="text-xl mb-2">{TYPE_ICONS[type] ?? "📦"}</div>
                <p className="text-xs font-semibold uppercase tracking-wide capitalize mb-1" style={{ color: "var(--e-text-2)" }}>{type}</p>
                <p className="text-lg font-bold" style={{ color: "var(--e-text-1)" }}>{items.length}</p>
                <p className="text-xs mt-0.5" style={{ color: "var(--e-text-3)" }}>{fmt(totalRevenue)}</p>
              </div>
            );
          })}
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 rounded-xl border" style={{ background: "var(--e-bg-surface)", borderColor: "var(--e-border)" }}>
          ...
          <p className="text-sm font-medium" style={{ color: "var(--e-text-1)" }}>No services yet</p>
          <p className="text-xs mt-1" style={{ color: "var(--e-text-3)" }}>Your booked services will appear here once students are enrolled.</p>
        </div>
      ) : (
        Object.entries(byType).map(([type, items]) => (
          <div key={type} className="rounded-xl border overflow-hidden" style={{ background: "var(--e-bg-surface)", borderColor: "var(--e-border)" }}>
            <div className="flex items-center gap-2 px-5 py-3" style={{ borderBottom: "1px solid #E8E6E2", background: "var(--e-bg-page)" }}>
              ...
              <h3 className="text-sm font-semibold capitalize" style={{ color: "var(--e-text-1)" }}>{type}</h3>
              <span className="ml-auto text-xs" style={{ color: "var(--e-text-3)" }}>{items.length} item{items.length !== 1 ? "s" : ""}</span>
            </div>
                ...
                <thead>
                  <tr style={{ borderBottom: "1px solid #F4F3F1" }}>
                    {["Service Name", "Student", "Dates", "Revenue (AP)", "Payment"].map(h => (
                      <th key={h} className="text-left px-5 py-2.5 text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--e-text-3)" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {items.map(b => (
                    <tr key={b.id} style={{ borderBottom: "1px solid #F4F3F1" }}
                      ...
                      <td className="px-5 py-3.5 font-medium" style={{ color: "var(--e-text-1)" }}>{b.name ?? "—"}</td>
