...
function contractStatusStyle(status: string | null | undefined) {
  ...
  if (s === "draft") return { background: "#F4F3F1", color: "#57534E", border: "1px solid #E8E6E2" };
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="mb-6 flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "#1C1917" }}>My Bookings</h1>
          <p className="text-sm mt-0.5" style={{ color: "#A8A29E" }}>Student bookings linked to your account</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 rounded-xl border" style={{ background: "#FFFFFF", borderColor: "#E8E6E2" }}>
          ...
          <p className="text-sm font-semibold" style={{ color: "#1C1917" }}>No bookings yet</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ borderBottom: "1px solid #E8E6E2", background: "#FAFAF9" }}>
                  <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wide" style={{ color: "#A8A29E" }}>Booking</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wide" style={{ color: "#A8A29E" }}>Student</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wide" style={{ color: "#A8A29E" }}>Program Dates</th>
                  <th className="text-right px-5 py-3 text-xs font-semibold uppercase tracking-wide" style={{ color: "#A8A29E" }}>Revenue (AP)</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wide" style={{ color: "#A8A29E" }}>Payment</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wide" style={{ color: "#A8A29E" }}>Status</th>
                </tr>
              </thead>
              <tbody>
                {bookings.map((b) => (
                  <tr key={b.id} style={{ borderBottom: "1px solid #F4F3F1" }}
                    ...
                    <td className="px-5 py-4">
                      <p className="font-medium" style={{ color: "#1C1917" }}>{b.name ?? "—"}</p>
                      {b.contractNumber && (
                        <p className="text-xs mt-0.5" style={{ color: "#A8A29E" }}>{b.contractNumber}</p>
                      )}
                      {b.serviceModuleType && (
                        <p className="text-xs capitalize" style={{ color: "#A8A29E" }}>{b.serviceModuleType}</p>
                      )}
                    </td>
                    <td className="px-5 py-4">
                      <p className="font-medium" style={{ color: "#1C1917" }}>{b.studentName ?? "—"}</p>
                      {b.clientEmail && <p className="text-xs" style={{ color: "#A8A29E" }}>{b.clientEmail}</p>}
                      {b.agentName && <p className="text-xs" style={{ color: "#A8A29E" }}>Agent: {b.agentName}</p>}
                    </td>
                    ...
                    <td className="px-5 py-4 text-right font-semibold" style={{ color: "#1C1917" }}>
                      {fmt(b.apAmount)}
                    </td>
                    <td className="px-5 py-4">
