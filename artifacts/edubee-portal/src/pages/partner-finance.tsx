  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Total Revenue", value: summary?.totalRevenue ?? 0, icon: DollarSign, color: "#F5821F", bg: "#FEF0E3" },
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                {isLoading
                  ...
                  : <p className="text-lg font-bold mt-1" style={{ color: "#1C1917" }}>{fmt(c.value)}</p>
                }
              </div>
      ...
      <div className="rounded-xl border overflow-hidden" style={{ background: "#FFFFFF", borderColor: "#E8E6E2" }}>
        <div className="flex items-center gap-2 px-5 py-3" style={{ borderBottom: "1px solid #E8E6E2", background: "#FAFAF9" }}>
          <Wallet size={15} style={{ color: "#A8A29E" }} />
          <h3 className="text-sm font-semibold" style={{ color: "#1C1917" }}>Payment Schedule</h3>
          <span className="ml-auto text-xs" style={{ color: "#A8A29E" }}>{bookings.length} item{bookings.length !== 1 ? "s" : ""}</span>
        </div>
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-sm" style={{ color: "#A8A29E" }}>No payments recorded yet.</p>
          </div>
        ) : (
                ...
                {bookings.map(b => (
                  <tr key={b.id} style={{ borderBottom: "1px solid #F4F3F1" }}
                    ...
                    <td className="px-5 py-3.5">
                      ...
                      {b.contractNumber && <p className="text-xs" style={{ color: "#A8A29E" }}>{b.contractNumber}</p>}
                    </td>
