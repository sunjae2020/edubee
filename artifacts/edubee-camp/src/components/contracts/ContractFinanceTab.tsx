...
const CC_BADGE: Record<string, string> = {
  "RC-CAMP":       "bg-[#FEF0E3] text-[#F5821F]",
  "RC-DIRECT":     "bg-[#FEF0E3] text-[#F5821F]",
...
const CC_BAR_COLOR: Record<string, string> = {
  "RC-CAMP": "bg-[#F5821F]", "RC-DIRECT": "bg-[#F5821F]",
  "CC-AGENT": "bg-[#DC2626]",
  default: "bg-[#A8A29E]",
};
    ...
    <div className="flex items-center justify-between px-4 py-3 border-b border-[#E8E6E2] bg-[#FAFAF9]">
      ...
      {onAdd && (
        <Button size="sm" variant="outline" onClick={onAdd}
          className="h-7 gap-1 text-xs border-[#F5821F] text-[#F5821F] hover:bg-[#FEF0E3]">
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-sm">
            ...
            Confirm Payment — <span className="text-[#F5821F]">{item?.label}</span>
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-3 mt-1">
          ...
          <div className="flex justify-end gap-2 pt-1 border-t">
            ...
            <Button size="sm"
              className="bg-[#F5821F] hover:bg-[#d97706] text-white min-w-[150px] gap-1.5"
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-sm">
            {mode === "add"
              ? <><Plus className="w-4 h-4 text-[#F5821F]" /> Add Finance Item</>
              : <><Pencil className="w-4 h-4 text-[#F5821F]" /> Edit Finance Item</>}
          </DialogTitle>
        </DialogHeader>
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <SummaryCard label="Net Camp Revenue" amount={summary.netCampRevenue ?? 0}
              icon={<DollarSign className="w-3.5 h-3.5 text-[#F5821F]" />}
              amountClass="text-[#F5821F]" />
            <SummaryCard
              label="Gross Profit"
          ...
          <div className="flex justify-end">
            <button onClick={() => generateMutation.mutate()} disabled={generateMutation.isPending}
              className="flex items-center gap-1.5 text-xs text-[#57534E] hover:text-[#F5821F] transition-colors">
                  ...
                  <tbody>
                    ...
                    <tr className="border-t-2 border-[#E8E6E2] bg-[#FAFAF9]">
                      ...
                      <td className={cn("px-4 py-2.5 text-right font-mono text-sm font-bold",
                        netBalance >= 0 ? "text-[#F5821F]" : "text-[#DC2626]")}>
