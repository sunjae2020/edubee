              ...
              {provider ? (
                ...
                >
                  <Avatar className="h-9 w-9 shrink-0">
                    <AvatarFallback className="bg-[#F5821F]/10 text-[#F5821F] text-xs font-bold">{providerInitials}</AvatarFallback>
                  </Avatar>
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-foreground group-hover:text-[#F5821F] transition-colors truncate">
                      {provider.fullName ?? "—"}
                    </p>
        ...
        {canEdit && (
          <div className="px-6 py-4 border-t border-border flex items-center justify-end gap-2 shrink-0 bg-background">
            {editing && !isCreate ? (
              ...
              <Button size="sm" className="bg-[#F5821F] hover:bg-[#D96A0A] text-white" onClick={() => setEditing(true)}>
                Edit
              </Button>
            )}
          </div>
