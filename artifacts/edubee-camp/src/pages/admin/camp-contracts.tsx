...
export default function CampContracts() {
  ...
  const queryKey = ["camp-contracts", { search, status: activeStatus, page }];
  ...
  const { data: targetContract } = useQuery({
    queryKey: ["camp-contract-by-id", targetContractId],
  ...
  const { data: services, isLoading: servicesLoading } = useQuery({
    queryKey: ["camp-contract-services", selected?.id],
  ...
  const { data: accounting, isLoading: accountingLoading } = useQuery({
    queryKey: ["camp-contract-accounting", selected?.id],
  ...
  const updateContract = useMutation({
    ...
    onSuccess: (updated) => {
      qc.invalidateQueries({ queryKey: ["camp-contracts"] });
