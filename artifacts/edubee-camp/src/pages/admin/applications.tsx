...
const STATUS_STYLES: Record<string, string> = {
  submitted:  "bg-[#FEF0E3] text-[#F5821F]",
...
function ServiceChip({ types }: { types?: unknown }) {
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {arr.map(t => (
        <span key={t} className="px-1.5 py-0.5 rounded text-[11px] font-medium bg-[#F5821F]/10 text-[#F5821F]">
          {t.replace(/_/g, " ")}
        </span>
      ))}
    </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <KpiCard label="Submitted" value={stats?.submitted ?? 0} color="text-[#F5821F]" />
