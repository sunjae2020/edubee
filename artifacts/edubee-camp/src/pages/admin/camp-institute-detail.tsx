...
import {
  ArrowLeft, School, DollarSign, Pencil, ExternalLink,
  Loader2, Check, X,
} from "lucide-react";
...
function OverviewTab({ record, canEdit, onSave }: { record: any; canEdit: boolean; onSave: (p: object) => void }) {
  ...
  const handleCancel = () => {
    setForm({ className: record.className ?? "", classLevel: record.classLevel ?? "", teacherName: record.teacherName ?? "", teacherComment: record.teacherComment ?? "", status: record.status ?? "pending", notes: record.notes ?? "", retailPrice: record.retailPrice ?? "", partnerCost: record.partnerCost ?? "" });
    setEditing(false);
  };
      ...
      <div className="rounded-xl border border-[#E8E6E2] bg-white p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-[#1C1917]">Institute Service</span>
          </div>
          {canEdit && !editing && (
        ...
        {editing ? (
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1 col-span-2">
              ...
              <Input value={form.className} onChange={e => f("className")(e.target.value)} className="h-8 text-sm" />
            </div>
            <div className="space-y-1">
              <Label className="text-xs">Class Level</Label>
              <Input value={form.classLevel} onChange={e => f("classLevel")(e.target.value)} className="h-8 text-sm" />
            </div>
            <div className="space-y-1">
            ...
            <div className="space-y-1 col-span-2">
              ...
              <Input value={form.teacherName} onChange={e => f("teacherName")(e.target.value)} className="h-8 text-sm" />
            </div>
            <div className="space-y-1 col-span-2">
              <Label className="text-xs">Teacher Comment</Label>
              <Textarea value={form.teacherComment} onChange={e => f("teacherComment")(e.target.value)} className="text-sm min-h-[60px]" />
            </div>
            <div className="space-y-1 col-span-2">
          ...
          <div className="grid grid-cols-2 gap-x-6">
            <InfoRow label="Class Name" value={record.className} />
            <InfoRow label="Class Level" value={record.classLevel} />
            <InfoRow label="Teacher" value={record.teacherName} />
            <InfoRow label="Status" value={<StatusBadge status={record.status} />} />
            <InfoRow label="Certificate Issued" value={record.certificateIssued ? "Yes" : "No"} />
            {record.teacherComment && <InfoRow label="Teacher Comment" value={record.teacherComment} />}
      ...
      <div className="rounded-xl border border-[#E8E6E2] bg-white p-4">
        ...
        {editing ? (
          ...
          <div className="grid grid-cols-2 gap-x-6">
            ...
            <InfoRow label="CoA AP" value={record.coaApCode ?? "4700"} />
          </div>
        )}
      </div>
    </div>
...
export default function CampInstituteDetail() {
  ...
  if (!data) return <div className="p-6 text-[#57534E]">Institute service not found.</div>;
    ...
    <div className="min-h-screen bg-[#FAFAF9]">
      ...
      <div className="bg-white border-b border-[#E8E6E2] px-6 py-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-[#1C1917]">{data.className ?? "Language School Service"}</h1>
      ...
      <div className="max-w-3xl mx-auto p-6">
        ...
        {activeTab === "documents" && (
          <EntityDocumentsTab entityType="camp_institute" entityId={id!} mode="full" />
        )}
        {activeTab === "notes" && (
