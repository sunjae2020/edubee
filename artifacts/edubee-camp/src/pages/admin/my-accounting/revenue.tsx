  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-3 mb-2">
        <div className="w-9 h-9 rounded-lg bg-[#F08301]/10 flex items-center justify-center"><BarChart3 className="w-5 h-5 text-[#F08301]" /></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {isLoading ? [...Array(3)].map((_, i) => <Skeleton key={i} className="h-24" />) : [
          ...
          },
          {
            ...
            icon: <DollarSign className="w-4 h-4 text-[#F08301]" />,
            color: "text-[#F08301]",
          },
        ].map(k => (
      ...
      <div className="bg-white rounded-xl border p-5">
        <h2 className="text-sm font-semibold mb-4 flex items-center gap-2"><BarChart3 className="w-4 h-4 text-[#F08301]" /> Monthly Revenue (Net)</h2>
              ...
              return (
                <div key={m.month} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 bg-muted/30 rounded-full h-6 overflow-hidden">
                    <div className="h-full bg-[#F08301] rounded-full flex items-center justify-end pr-2 transition-all duration-500" style={{ width: `${Math.max(pct, 1)}%` }}>
      ...
      <div className="bg-white rounded-xl border p-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="text-center p-4 bg-yellow-50 rounded-lg border border-yellow-100">
            <div className="text-2xl font-bold text-yellow-700">A${Number(summaryData?.pending ?? 0).toLocaleString("en-AU", { minimumFractionDigits: 2 })}</div>
            <div className="text-xs text-yellow-600 mt-1">Pending Payout</div>
          </div>
          <div className="text-center p-4 bg-green-50 rounded-lg border border-green-100">
            <div className="text-2xl font-bold text-green-700">A${Number(summaryData?.paid ?? 0).toLocaleString("en-AU", { minimumFractionDigits: 2 })}</div>
