...
function StatusBadge({ status }: { status?: string | null }) {
  ...
  const map: Record<string, string> = {
    ...
    checked_in: "bg-[#FEF0E3] text-[#F5821F]", checked_out: "bg-[#DCFCE7] text-[#16A34A]", cancelled: "bg-[#FEF2F2] text-[#DC2626]",
  };
...
export default function HotelManagement() {
  ...
  const queryKey = ["services-hotel", { search, status: activeStatus, page, pageSize }];
    ...
    <div className="space-y-4">
      ...
      <Sheet open={!!selected} onOpenChange={o => { if (!o) { setSelected(null); setEditing(false); } }}>
        <SheetContent className="w-[520px] sm:max-w-[520px] overflow-y-auto bg-background">
          <SheetHeader>
            <SheetTitle className="flex items-center gap-2"><Building2 className="w-4 h-4 text-[#F5821F]" /> Hotel Record</SheetTitle>
