...
function StatusBadge({ status }: { status?: string | null }) {
  ...
  const map: Record<string, string> = {
    ...
    in_transit: "bg-[#FEF0E3] text-[#F5821F]", completed: "bg-[#DCFCE7] text-[#16A34A]", cancelled: "bg-[#FEF2F2] text-[#DC2626]",
  };
    ...
    <div className="space-y-4">
        ...
        <table className="w-full min-w-[920px] text-sm">
          ...
          <tbody className="divide-y divide-border">
            {isLoading ? (
              ...
              <tr key={r.id} className="hover:bg-[#FEF0E3] transition-colors cursor-pointer" onClick={() => navigate(`${BASE}/admin/services/pickup/${r.id}`)}>
      ...
      <Sheet open={!!selected} onOpenChange={o => { if (!o) { setSelected(null); setEditing(false); } }}>
        <SheetContent className="w-[520px] sm:max-w-[520px] overflow-y-auto bg-background">
          <SheetHeader>
            <SheetTitle className="flex items-center gap-2"><Car className="w-4 h-4 text-[#F5821F]" /> Pickup Record</SheetTitle>
