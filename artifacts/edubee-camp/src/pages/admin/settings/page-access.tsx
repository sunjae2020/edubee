...
export default function PageAccess() {
  ...
  const tabBtnStyle = (active: boolean): React.CSSProperties => ({
    ...
    borderBottom: active ? "2px solid var(--e-orange)" : "2px solid transparent",
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-[--e-orange]/10 flex items-center justify-center">
            <Lock className="w-5 h-5 text-[--e-orange]" />
          </div>
          <div>
        ...
        {activeTab === "access" && (
          <Button size="sm" className="bg-[--e-orange] hover:bg-[#d97706] text-white gap-1.5" onClick={handleSave}>
      ...
      {activeTab === "access" && (
        <>
                ...
                <tr className="border-b bg-muted/30">
                  ...
                  {ROLES.map(r => (
                    <th key={r.key} className="px-2 py-3 text-center text-[11px] font-semibold text-muted-foreground uppercase tracking-wide" style={{ minWidth: 72 }}>
                      ...
                      {r.locked && <Lock className="w-3 h-3 mx-auto mt-0.5 text-[--e-orange]" />}
                    </th>
                  ))}
                </tr>
              </thead>
                ...
                {GROUPS.map(group => (
                  <React.Fragment key={group}>
                    ...
                    {PAGES.filter(p => p.group === group).map(p => (
                      <tr key={p.slug} className="border-t hover:bg-[--e-orange-lt]">
                          ...
                          <td key={r.key} className="px-2 py-2 text-center">
                            <button
                              ...
                              className={`w-5 h-5 rounded border-2 flex items-center justify-center mx-auto transition-all ${
                                r.locked
                                  ? "bg-[--e-orange] border-[--e-orange] cursor-not-allowed"
                                  : matrix[p.slug]?.[r.key]
                                  ? "bg-[--e-orange] border-[--e-orange] hover:bg-[#d97706]"
                                  : "border-muted-foreground/30 hover:border-[--e-orange]/50"
                              }`}
                            >
          ...
          <div className="flex justify-end sticky bottom-4">
            <Button className="bg-[--e-orange] hover:bg-[#d97706] text-white gap-1.5 shadow-lg" onClick={handleSave}>
