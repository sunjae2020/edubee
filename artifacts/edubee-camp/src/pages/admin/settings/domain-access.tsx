    ...
    <div className="space-y-4">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 space-y-3">
        <p className="text-sm font-semibold text-blue-800 flex items-center gap-2">
          <Info size={14} /> How to set up CNAME/TXT records
        </p>
        <ol className="space-y-2 text-xs text-blue-700 list-none">
          {[
            ...
            "Add the CNAME and TXT records below individually.",
      ...
      {(cname || txt) ? (
                ...
                {cname && (
                  <tr>
                    ...
                    <td className="py-3 px-4">
                      ...
                      <p className="text-[11px] text-[#A8A29E] mt-0.5">Points domain root to Edubee</p>
                    </td>
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-1.5">
                ...
                {txt && (
                  <tr>
                    ...
                    <td className="py-3 px-4">
                      ...
                      <p className="text-[11px] text-[#A8A29E] mt-0.5">Verifies domain ownership</p>
                    </td>
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-1.5">
        ...
        <div className="flex items-start gap-2 p-3 rounded-lg bg-[#F4F3F1] text-sm text-[#57534E]">
          ...
          <p>Register a custom domain first to view DNS records.</p>
        </div>
      )}
    </div>
  );
...
function NameserverSection({ customDomain, toast }: { customDomain?: string; toast: any }) {
  const registrarSteps = [
    ...
    { label: "Domain management → Change nameservers", desc: "Find the 'Nameservers' or 'DNS Servers' section on your domain management page." },
    { label: "Delete all existing nameservers and enter the values below", desc: "Select the Custom/Other nameserver option and enter the two values below one by one." },
    { label: "Save and wait for propagation", desc: "Nameserver changes can take up to 48 hours. Once complete, your domain will automatically connect to Edubee." },
  ];
    ...
    <div className="space-y-4">
      ...
      <div className="bg-purple-50 border border-purple-200 rounded-xl p-4 space-y-3">
        <p className="text-sm font-semibold text-purple-800 flex items-center gap-2">
          <Server size={14} /> How to set up Nameservers (NS)
        </p>
        <p className="text-xs text-purple-700 leading-relaxed">
          Changing your domain's nameservers to Edubee connects it automatically without
          needing to add DNS records manually. Simpler than CNAME/TXT, but
          <strong> existing DNS records (email MX, etc.) will be reset</strong>.
        </p>
      </div>
      ...
      <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-50 border border-amber-200">
        ...
        <div className="text-xs text-amber-700 space-y-1">
          ...
          <p>If you are using an email service, we recommend the <strong>CNAME/TXT method</strong> instead.</p>
        </div>
      </div>
    ...
    <div className="max-w-3xl mx-auto space-y-6 py-6 px-4">
      ...
      <div className="bg-white rounded-xl border border-[#E8E6E2] p-6 space-y-5" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
        ...
        <div className="flex items-start gap-2 p-3 rounded-lg bg-[#F0FDF4] border border-[#A7F3D0] text-sm text-[#166534]">
          <Info size={14} className="mt-0.5 shrink-0" />
          <p>Subdomains (e.g. <span className="font-mono font-semibold">abc.edubee.co</span>) are managed automatically by Edubee. No DNS configuration required.</p>
        </div>
        ...
        {availability === true && (
          <div className="space-y-3">
            <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-50 border border-amber-200">
              <AlertTriangle size={14} className="text-amber-600 mt-0.5 shrink-0" strokeWidth={1.5} />
              <p className="text-xs text-amber-700">Changing the subdomain will break all existing links using the current URL.</p>
            </div>
            <button
              onClick={saveDomain}
              disabled={saving}
        ...
        {!canCustomDomain ? (
          ...
          <>
              ...
              <div>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <MethodTab
                    ...
                    description="Change your domain's nameservers entirely to Edubee. Simple setup, but resets existing DNS."
                  />
                </div>
              </div>
            ...
            {domain?.customDomain && !cname && !txt && dnsMethod === "cname" && (
              <div className="flex items-start gap-2 p-3 rounded-lg bg-[#F4F3F1] text-sm text-[#57534E]">
                <Info size={14} className="mt-0.5 shrink-0" />
                <p>DNS records are being prepared. Please refresh the page shortly.</p>
              </div>
            )}
          </>
        )}
