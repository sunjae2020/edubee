...
const TYPE_BADGE: Record<string, string> = {
  ...
  Organisation: "bg-[#FEF0E3] text-[#F5821F]",
};
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="shrink-0 p-1.5 rounded hover:bg-[#FEF0E3] text-[#A8A29E] hover:text-[#F5821F] transition-colors"
      title="Copy URL"
    >
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg space-y-5 overflow-y-auto max-h-[90vh] p-6">
        ...
        {!isEdit && (
          <div className="space-y-1.5 relative">
            ...
            {selectedAccount ? (
              <div className="flex items-center gap-2 px-3 py-2 bg-[#FEF0E3] rounded-lg border border-[#F5821F]/30">
          ...
          {[
            ...
            <label key={opt.value} className="flex items-start gap-3 cursor-pointer">
              <input type="radio" name="emailNotif" value={opt.value} checked={emailNotif === opt.value}
                onChange={() => setEmailNotif(opt.value)} className="mt-0.5 accent-[#F5821F]" />
              <div>
        ...
        <div className="flex items-center justify-between py-1">
          ...
          <button
            ...
            className={`relative w-11 h-6 rounded-full transition-colors ${isActive ? "bg-[#F5821F]" : "bg-[#E8E6E2]"}`}
          >
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-[#E8E6E2] px-6 py-4">
        <div className="flex items-center gap-2 text-sm mb-1">
          <button
            ...
            className="text-[#A8A29E] hover:text-[#F5821F] transition-colors"
          >
            Application Forms
          </button>
        ...
        <div className="bg-white rounded-xl border border-[#E8E6E2] overflow-hidden">
          {partnersLoading ? (
            <div className="flex items-center justify-center h-32">
              <Loader2 className="w-5 h-5 animate-spin text-[#F5821F]" />
            </div>
          ) : partners.length === 0 ? (
                      ...
                      <tr key={p.id} className={`border-b border-[#E8E6E2] hover:bg-[#FAFAF9] transition-colors ${i === partners.length - 1 ? "border-b-0" : ""}`}>
                        ...
                        <td className="px-4 py-3">
                          <button
                            ...
                            className={`relative w-10 h-5 rounded-full transition-colors ${p.isActive ? "bg-[#F5821F]" : "bg-[#E8E6E2]"}`}
                          >
