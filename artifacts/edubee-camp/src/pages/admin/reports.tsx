    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-[#F08301]/10 flex items-center justify-center"><BookOpen className="w-5 h-5 text-[#F08301]" /></div>
      ...
      {canEdit && (
        <div className="grid grid-cols-2 gap-3 mb-4">
          ...
          <div className="bg-white rounded-lg border p-3 text-center">
            <div className="text-2xl font-bold text-green-600">{published.length}</div>
      ...
      {isLoading ? (
        ...
        <div className="grid gap-3">
          {reports.map(r => (
            <div key={r.id} className="bg-white rounded-lg border p-4 hover:border-[#F08301]/40 transition-colors cursor-pointer flex items-start justify-between gap-3" onClick={() => setSelected(r)}>
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold border capitalize ${STATUS_COLORS[r.status ?? "draft"] ?? "bg-gray-100 text-gray-700 border-gray-200"}`}>{r.status}</span>
              ...
              <div className="flex gap-2 shrink-0" onClick={e => e.stopPropagation()}>
                ...
                {canEdit && r.status === "draft" && (
                  <Button size="sm" className="h-7 text-xs bg-green-600 hover:bg-green-700 text-white gap-1" onClick={() => publishMutation.mutate(r.id)}>
      ...
      <Sheet open={!!selected} onOpenChange={o => { if (!o) setSelected(null); }}>
        <SheetContent className="w-[560px] sm:max-w-[560px] overflow-y-auto">
          <SheetHeader><SheetTitle className="flex items-center gap-2"><BookOpen className="w-4 h-4 text-[#F08301]" /> {selected?.reportTitle ?? "Report"}</SheetTitle></SheetHeader>
          {selected && (
            <div className="mt-4 space-y-4">
              <div className="flex items-center gap-2">
                <span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold border capitalize ${STATUS_COLORS[selected.status ?? "draft"] ?? "bg-gray-100 text-gray-700 border-gray-200"}`}>{selected.status}</span>
              ...
              <div className="flex gap-2 pt-2">
                {canEdit && selected.status === "draft" && (
                  <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white gap-1.5" onClick={() => publishMutation.mutate(selected.id)} disabled={publishMutation.isPending}>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3 mt-2">
            ...
            <div className="flex gap-2 pt-1">
              <Button size="sm" className="flex-1 bg-[#F08301] hover:bg-[#d97706] text-white" onClick={() => createMutation.mutate(form)} disabled={createMutation.isPending || !form.reportTitle}>
