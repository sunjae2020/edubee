...
router.get("/dashboard/v2/overview", authenticate, requireRole(...ADMIN_ROLES), async (req, res) => {
  try {
    const [
      ...
      scalar(sql`SELECT COUNT(*)::int AS val FROM leads WHERE status NOT IN ('closed_won','closed_lost','cancelled','converted')`),
...
router.get("/dashboard/v2/operation", authenticate, requireRole(...ADMIN_ROLES), async (req, res) => {
  try {
    ...
    const upcomingInterviewsRows = await rows(sql`
      SELECT
        i.id,
        COALESCE(l.full_name, 'Unknown')    AS "contactName",
        UPPER(LEFT(COALESCE(l.full_name, '?'), 2)) AS initials,
        NULL::text                           AS "schoolName",
        i.format                             AS "interviewType",
        i.scheduled_datetime                 AS "scheduledAt"
      FROM interview_schedules i
      LEFT JOIN applications a ON a.id = i.application_id
      LEFT JOIN leads l ON l.id = a.id
    ...
    const recentActivityRows = await rows(sql`
      SELECT id, 'lead' AS type,
        'Lead updated: ' || full_name AS description,
...
router.get("/dashboard/v2/sales", authenticate, requireRole(...ADMIN_ROLES), async (req, res) => {
  try {
    ...
    const pipeline = {
      new:         (statusMap["new"] ?? 0),
      inProgress:  (statusMap["in_progress"] ?? 0) + (statusMap["open"] ?? 0),
    ...
    const recentQuotesRows = await rows(sql`
      SELECT
        ...
        NULL::text AS "schoolName",
        COALESCE((SELECT SUM(qp.total) FROM quote_products qp WHERE qp.quote_id = q.id AND qp.status = 'Active'), 0)::numeric AS "totalAmount",
    ...
    return res.json({
      pipeline,
      monthlyLeads: monthlyLeadsRows.map((r: any) => ({ month: r.month, year: r.year, count: Number(r.count) })),
      leadSources,
  ...
  try {
    ...
    const commissionsRows = await rows(sql`
      SELECT
        ...
        COALESCE(l.full_name, c.title, 'Unknown')     AS "studentName",
    ...
    const totalExpectedCommission = await scalar(sql`
      SELECT COALESCE(SUM(ap_amount),0)::numeric AS val FROM contract_products WHERE ap_status IN ('pending','ready')
    `);
    ...
    return res.json({
      ...
      totalIncentive: Number(kpiRow?.total_incentive ?? 0),
      monthlyRevenue: monthlyRevenueRows.map((r: any) => ({ month: r.month, year: r.year, ar: Number(r.ar), ap: Number(r.ap) })),
      arSummary: {
        total:        arTotal,
        outstanding:  arOutstanding,
        overdueCount: arOverdueCount,
        invoiceCount: arInvoiceCount,
      },
      apSummary: {
        total:        apTotal,
        overdue:      apOverdue,
        urgentCount:  apUrgentCount,
        payableCount: apPayableCount,
      },
