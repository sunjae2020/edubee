...
router.get("/contracts/:id/finance", authenticate, async (req, res) => {
  try {
    ...
    const data = await buildFinanceSummary(req.params.id as string);
  ...
  async (req, res) => {
    try {
      ...
      const [updated] = await db
        ...
        .set(updates)
        .where(and(eq(contractFinanceItems.id, req.params.itemId as string), eq(contractFinanceItems.isDeleted, false)))
        .returning();
  ...
  async (req, res) => {
    try {
      const [updated] = await db
        ...
        .where(eq(contractFinanceItems.id, req.params.itemId as string))
        .returning();
      if (!updated) return res.status(404).json({ error: "Finance item not found" });
  ...
  async (req, res) => {
    try {
      const entries = await db
        .select()
        .from(userLedger)
        .where(eq(userLedger.contractId, req.params.id as string))
...
router.get(
  ...
  async (req, res) => {
    try {
      const { receiptId } = req.params as Record<string, string>;
  ...
  async (req, res) => {
    try {
      ...
      const items = await db.select({ id: cfi.id }).from(cfi)
        .where(and(eq(cfi.contractId, req.params.id as string), eq(cfi.isDeleted, false)));
